...
static const char azureSphereModelId[] = "";
...
Cloud_Result Cloud_SendTelemetry(const Cloud_Telemetry *telemetry, time_t timestamp)
{
    ...
    json_object_dotset_number(telemetryRoot, "co2", telemetry->co2);
    json_object_dotset_number(telemetryRoot, "tvoc", telemetry->tvoc);
