...
static const char dps_url[] = "global.azure-devices-provisioning.net";
...
ExitCode Dps_Connection_Initialize(EventLoop* el, Connection_StatusCallbackType statusCallBack, ExitCode_CallbackType failureCallback, const char* modelId, void* context)
{
    failureCallbackFunction = failureCallback;
    connectionStatusCallback = statusCallBack;

    Dps_Connection_Config* config = context;
...
void Dps_Connection_Cleanup(void) {}
...
static bool Dps_IsReadyToProvision(void)
{
...
#define MAX_DTDL_BUFFER_SIZE (15 + MAX_MODELID_LENGTH + 1)
    ...
    PROV_DEVICE_RESULT prov_result = Prov_Device_LL_SetOption(provHandle, "SetDeviceId", &deviceIdForDaaCertUsage);
    ...
    prov_result = Prov_Device_LL_Register_Device(provHandle, RegisterDeviceCallback, NULL, NULL, NULL);
    if (prov_result != PROV_DEVICE_RESULT_OK) {
...
static void OnRegisterComplete(void)
{
    ...
    iothubClientHandle = IoTHubDeviceClient_LL_CreateWithAzureSphereFromDeviceAuth(iotHubUri, &MQTT_Protocol);
    ...
    // IoT Hub client.
    static const int deviceIdForDaaCertUsage = 1;
    IOTHUB_CLIENT_RESULT iothubResult = IoTHubDeviceClient_LL_SetOption(iothubClientHandle, "SetDeviceId", &deviceIdForDaaCertUsage);
    ...
    if ((iothubResult = IoTHubDeviceClient_LL_SetOption(
        iothubClientHandle, OPTION_AUTO_URL_ENCODE_DECODE, &urlAutoEncodeDecode)) != IOTHUB_CLIENT_OK) {
